...
from MobileFaceNet.utils.util import *
from MobileFaceNet.utils.align_trans import *
from MTCNN import create_mtcnn_net
from MobileFaceNet.models.face_model import MobileFaceNet, l2_norm
from MobileFaceNet.utils.facebank import load_facebank, prepare_facebank
import cv2
import time
...
if __name__ == '__main__':
    ...
    detect_model.load_state_dict(torch.load(str(weights_path), map_location=lambda storage, loc: storage))
    print('MobileFaceNet face detection model generated')
    detect_model.eval()

    facebank_path = PROJECT_ROOT / "facebank"
    if args.update:
        targets, names = prepare_facebank(detect_model, path=facebank_path, tta=args.tta)
        print('facebank updated')
    else:
        targets, names = load_facebank(path=facebank_path)
    ...
    font = ImageFont.truetype(str(PACKAGE_ROOT / "utils" / "simkai.ttf"), 20)
